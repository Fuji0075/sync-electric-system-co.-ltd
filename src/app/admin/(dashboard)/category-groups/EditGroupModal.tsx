...
type Group = { id: string; name: string; slug: string; order: number };
...
export default function EditGroupModal({
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="text-sm text-emerald-400 hover:underline">
        แก้ไข
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setOpen(false)}>
          ...
          >
            <div className="flex items-center justify-between border-b border-[var(--admin-border)] px-5 py-3">
              <span className="text-sm font-semibold text-[var(--admin-text-secondary)]">แก้ไขหมวดหมู่หลัก</span>
              <button
                type="button"
            ...
            <form action={handleSubmit} className="space-y-4 px-5 py-5">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-[var(--admin-text-secondary)]">Slug</label>
                <input
                  name="slug"
