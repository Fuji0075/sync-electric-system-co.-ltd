...
export default function PermissionCheckboxes({
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2 rounded-lg border border-white/10 p-3 sm:grid-cols-3">
        {MODULES.map((m) => (
          <label key={m.key} className="flex items-center gap-2 text-sm text-zinc-300">
            <input
              type="checkbox"
              name={`perm_${m.key}`}
