...
export default async function AdminDashboardPage() {
  ...
  return (
    <div>
      ...
      <h1 className="mb-6 text-xl font-bold text-[var(--admin-text)]">แดชบอร์ด</h1>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {visibleCards.map((c) => (
          <Link
            key={c.label}
            href={c.href}
            className={`flex items-center gap-4 rounded-2xl border bg-[var(--admin-surface)] p-5 shadow-sm transition hover:shadow-md ${
              c.highlight
                ? "border-red-500/40 hover:border-red-400"
                : "border-[var(--admin-border)] hover:border-brand"
            }`}
          >
            ...
            <div>
              <div
                className={`text-2xl font-extrabold ${
                  c.highlight ? "text-red-400" : "text-[var(--admin-text)]"
                }`}
              >
                {c.value}
              </div>
              <div className="text-xs text-[var(--admin-text-faint)]">{c.label}</div>
            </div>
          </Link>
        ))}
      </div>
