...
export default function ChatSidebar({ conversations }: { conversations: SidebarConversation[] }) {
  ...
  return (
    <aside className="flex w-full max-w-xs shrink-0 flex-col border-r border-neutral-200 bg-white">
      <div className="border-b border-neutral-100 p-4">
        <h1 className="text-lg font-bold text-neutral-900">แชทกับลูกค้า</h1>
        <div className="relative mt-3">
          ...
          <input
            ...
            className="w-full rounded-full border border-neutral-200 bg-neutral-50 py-2 pl-9 pr-3 text-sm outline-none focus:border-brand focus:bg-white"
          />
        </div>
      </div>

      {needsAttention.length > 0 && (
        <div className="flex gap-3 overflow-x-auto border-b border-neutral-100 p-3">
          {needsAttention.slice(0, 8).map((c) => (
            ...
            >
              ...
              <span className="max-w-[3.5rem] truncate text-[10px] text-neutral-500">
      ...
      <p className="px-4 pt-3 text-xs font-semibold uppercase tracking-wide text-neutral-400">
        ล่าสุด
      </p>
      <ul className="flex-1 divide-y divide-neutral-100 overflow-y-auto">
        ...
        {filtered.length === 0 && (
          <li className="p-8 text-center text-sm text-neutral-400">ไม่พบการสนทนา</li>
        )}
      </ul>
    </aside>
  );
