...
export default async function AdminQuotesPage() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-xl font-bold text-[var(--admin-text)]">คำขอใบเสนอราคา</h1>

      <div className="space-y-4">
        {quotes.map((q) => (
          <div key={q.id} className="rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                <p className="font-semibold text-[var(--admin-text)]">{q.productName ?? "สอบถามทั่วไป"}</p>
                <p className="text-sm text-[var(--admin-text-muted)]">
                  {q.name} · {q.email} {q.phone && `· ${q.phone}`}
                </p>
                {q.company && <p className="text-sm text-[var(--admin-text-faint)]">บริษัท: {q.company}</p>}
                {q.quantity && <p className="text-sm text-[var(--admin-text-faint)]">จำนวน: {q.quantity}</p>}
                {q.message && (
                  <p className="mt-2 whitespace-pre-line text-sm text-[var(--admin-text-secondary)]">{q.message}</p>
                )}
              </div>
              <div className="text-right">
                <time className="block text-xs text-[var(--admin-text-faint2)]">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              >
                <button
                  type="submit"
                  className="rounded-full bg-brand px-3 py-1.5 text-xs font-semibold text-[var(--admin-text)] hover:bg-brand-dark"
                >
                  🧾 สร้างใบเสนอราคา
                </button>
              </form>
                ...
                >
                  <button
                    ...
                    className={`rounded-full px-3 py-1.5 text-xs font-semibold ${
                      q.status === value
                        ? "bg-brand text-[var(--admin-text)]"
                        : "border border-[var(--admin-border-strong)] text-[var(--admin-text-muted)] hover:border-brand hover:text-emerald-400"
                    }`}
                  >
                    {label}
                  </button>
